...
- **spec / case / why / link / rule 都挂 symbol**（ccr 评审一个改动 unit 收的**五类上下文**）：spec 记录契约，case 记录验证场景，why 记录代码无法表达的设计理由，link 指向策展的 see-also，rule 记录审查准则。why 可不依附 spec 独立存在；五类 marker 都可挂函数、方法或类/类型，供 review 按 symbol 回溯注入。见 `docs/concepts.md`。
