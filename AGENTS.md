...
- `toolchains/typescript/` — TypeScript decorator / JSDoc marker 包与基于 Compiler API 的 `specgen`。
...
- **spec / case / link / rule 都挂 symbol**（ccr 评审一个改动 unit 收的**四类上下文**）：一个符号 0..1 spec、0..N case、0..N link、0..N rule。symbol 可以是函数、方法，或**类/类型**。link = 作者策展的 "see also"（md 路径或另一 symbol-id）；rule = 审查准则（rule.json 的共置细化）——挂在函数上是"改它时盯什么"，挂在**类/类型**上是"用到该类型时盯什么"（类型级用法约束，如"仅 per-request"），供 review 在 diff 引用该类型时回溯注入。见 `docs/concepts.md`。
