...
可绑定的符号是函数、方法与类/类型。七个 marker（`spec`/`case`/`why`/`ideal`/`tmp`/`link`/`rule`）挂在其中任一符号上：挂在类/类型上绑定到 `<relpath>::<类型名>`，描述该类型整体（契约 / 用例 / 当前设计理由 / 理想形态 / 临时措施与退出条件 / see-also / 用法约束），与该类型内的方法符号同址不同 key。类级 `rule` 常表达"类型级用法约束"（如"仅 per-request"）。
